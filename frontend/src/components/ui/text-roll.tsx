...
export function TextRoll({
  ...
  const handleLastLetterComplete = useCallback(() => {
    if (!loop) {
      onAnimationComplete?.();
      return;
    }
    // Wait loopDelay seconds, then remount by incrementing cycle
  ...
  // Reset cycle on children change
  useEffect(() => {
    setCycle(0);
  }, [children]);
        ...
        return (
          ...
          >
            ...
            <motion.span
              ...
              onAnimationComplete={
                letters.length === i + 1 ? handleLastLetterComplete : undefined
              }
            >
              {letter === ' ' ? '\u00A0' : letter}
            </motion.span>
